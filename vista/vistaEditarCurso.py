from tkinter import CENTER, END, W, Frame, Entry, Label, Button, StringVar, ttk, messagebox

class EditarCurso:

    def __init__(self, ventana, database, ventanaAnterior, codigoCurso, ventanaInicial):
    ...
    def agregarEtiquetas(self):
        etiquetaTitulo = Label(self.panel, text="EDITAR CURSO", font=("Microsoft Yahei UI Light", 14, "bold"), bg="white", fg="#D35400", anchor=CENTER)
    ...
    def agregarCamposYCajas(self):
        ...
        self.cajaEstado.place(x=166, y=291, width=275, height=22)

    def agregarBotones(self):
        botonEditar = Button(self.panel, text="Editar", font=("Segoe UI", 11, "normal"), bg="#154360", fg="white", activebackground="#1F618D", activeforeground="white", cursor="hand2", bd=0, command=self.editarCurso)
        botonEditar.place(x=221, y=333, width=100, height=20)
    ...
    def llenarCampos(self):
        curso = self.db.getCurso(self.codigoCurso)
        self.txtcodigo.set(curso.getCodigo())
        self.txtnombre.set(curso.getNombre())
        self.txtprerrequisito.set(curso.getPrerrequisito())
        self.cajaOpcionalidad.current(self.cajaOpcionalidad["values"].index(curso.getOpcionalidad()))
        self.cajaSemestre.current(self.cajaSemestre["values"].index(curso.getSemestre()))
        self.txtcreditos.set(curso.getCreditos())
        self.cajaEstado.current(self.cajaEstado["values"].index(curso.getEstado()))
        self.campoCodigo.icursor(END)
        self.campoCodigo.focus()
    ...
    def editarCurso(self):
        if ((self.txtcodigo.get() and self.txtnombre.get() and self.txtcreditos.get()) != "") and (self.cajaOpcionalidad.get() != "Seleccionar" and self.cajaSemestre.get() != "Seleccionar" and self.cajaEstado.get() != "Seleccionar"):       
            if (self.txtcodigo.get().isdigit() and self.txtcreditos.get().isdigit()):
                self.db.updateCurso(self.txtcodigo.get(), self.txtnombre.get(), self.txtprerrequisito.get(), self.cajaOpcionalidad.get(), self.cajaSemestre.get(), self.txtcreditos.get(), self.cajaEstado.get())
                messagebox.showinfo("Informacion", "¡Curso actualizado exitosamente!")
                self.limpiarCampos()
