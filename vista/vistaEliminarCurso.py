from tkinter import E, Frame, Button, Entry, Label, StringVar, messagebox
...
class EliminarCurso:
    ...
    def agregarBotones(self):
        botonEliminar = Button(self.panel, text="Eliminar", font=("Segoe UI", 11, "normal"), bg="#154360", fg="white", activebackground="#1F618D", activeforeground="white", cursor="hand2", bd=0, command=self.eliminar)
        botonEliminar.place(x=210, y=70, width=75, height=20)
        ...
        if (self.txtcodigo.get() == ""):
            ...
            if (self.txtcodigo.get().isdigit()):
                if (self.db.searchCodigo(self.txtcodigo.get())):
                    self.db.deleteCurso(self.txtcodigo.get())
                    messagebox.showinfo("Informacion", "¡Curso eliminado exitosamente!")
                    self.txtcodigo.set("")
                else:
